...
for b = 1:length(bandgaps)
    for i = 1:no_simulations
        ...
        displacements_x = displacement_x_new(bandgaps(b), max(e_field), e_field);
        ...
        overall_current_x = brunel_current_density + kerr + injection_current_density;
        ...
        if i == 20
            figure(1)
            semilogy(P_overall_current(1:n_fft/2 + 1),'black');
            hold on
...
% semilogy(VarName1 .*10^16, E_geV, 'black');
% semilogy(VarName3 .*10^16, VarName3, 'black');
